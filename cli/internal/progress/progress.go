// Package progress provides a simple terminal progress bar that wraps an
// io.Reader and renders a bar to stderr as data flows through it.
package progress
...
// total is the expected byte count; pass 0 if unknown (shows bytes read instead).
...
// Finish forces the bar to 100% (call if EOF is handled outside Read loop).
...
// PrintDone prints a completed bar without wrapping an io.Reader.
// Useful when data is already in memory and the "progress" is network time.
func PrintDone() {
	bar := strings.Repeat("█", barWidth)
